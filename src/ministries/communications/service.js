...
const DB_PASS = 'communications_db_password_123';
const FLAG = 'FLAG{communications_flag_01}';
const BACKUP_FLAG = 'FLAG{communications_backup_01}';
const ADMIN_CREDS = {
    username: 'admin',
    password: 'admin123'
};
...
const INTERNAL_TOKEN = 'communications_internal_token_123';

// Database connection with hardcoded credentials
const db = mysql.createPool({
    host: 'localhost',
    user: 'communications_user',
    password: DB_PASS,
...
async function searchRecords(query) {
    ...
    const [rows] = await db.query(`SELECT * FROM messages WHERE content LIKE '%${query}%'`);
    return rows;
}

// CSRF vulnerability
async function updateMessage(messageId, status) {
...
function getLogs() {
    const logs = [
        { timestamp: '2024-03-20 10:00:00', action: 'Message update', user: 'officer1' },
        { timestamp: '2024-03-20 10:15:00', action: 'Message record access', user: 'admin' },
